 ...
 * @return： none
*/
void QB8::move_solution(float x_speed, float y_speed, int16_t angular_velocity)
{
...
{
  //直角坐标变换，矫正姿态传感器的放置误差
  stcAngle.Angle[0] = stcAngle.Angle[0] * cos(CORRECT_ANGLE* PI/ 180) - stcAngle.Angle[1] * sin(CORRECT_ANGLE* PI/ 180);
  stcAngle.Angle[1] = stcAngle.Angle[1] * cos(CORRECT_ANGLE* PI/ 180) + stcAngle.Angle[0] * sin(CORRECT_ANGLE* PI/ 180);
  //上一帧的脉冲频率作为速度
  give_x_speed = Pid_Out(&balance_x_pid, stcAngle.Angle[0], 0) +  Pid_Out(&speed_x_pid, give_x_speed, 0); 
  give_y_speed = Pid_Out(&balance_y_pid, stcAngle.Angle[1], 0) +  Pid_Out(&speed_y_pid, give_y_speed, 0);
